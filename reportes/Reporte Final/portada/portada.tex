\begin{titlepage}
	\centering
	%-------------------------------------------
	% Logos en una tabla: izquierda, centro y derecha
	\begin{tabular}{@{}p{0.3\textwidth} p{0.3\textwidth} p{0.3\textwidth}@{}}
		\includegraphics[height=2cm]{tecnm} & 
		\centering \includegraphics[height=1.5cm]{SEP} & 
		\raggedleft \includegraphics[height=2cm]{ith.jpg} \\
	\end{tabular}
	
	\vspace{2em}
	
	\noindent
	%-------------------------------------------
	%	Información institucional y académica (esquina superior izquierda)
	\begin{minipage}[t]{0.6\textwidth}
		\raggedright
		\small \textbf{%
			Instituto Tecnológico de Hermosillo\\
			Materia: Robótica\\
			Profesor: Medina Gil Lamadrid, Jesús Iván%
		}
	\end{minipage}%
	\hfill
	%	fecha actual (esquina superior derecha), en letras pequeñas y en negrita.
	\begin{minipage}[t]{0.3\textwidth}
		\raggedleft
		\small \textbf{\today}
	\end{minipage}
	
	\vspace{2em}
	
	%-----------------------------------------
	% Unidad y Título de la tarea en letras grandes y en negrita
%	{\large \textbf{Unidad Final}}\\
	{\Huge \textbf{Reporte final del Robot}}
		
	\vspace{1em}
	
	%---------------------------------------
	% Tabla con la información del equipo
	%---------------------------------------
	% Encabezado del equipo
	\begin{center}
		{\Large \textbf{Equipo 8}}
		
		{\small \textbf{https://github.com/Chiquito-22/Robotica.git}}
	\end{center}
	
	\vspace{1em}
	
	% Tabla de integrantes:
	% Cada fila contiene: foto (columna izquierda) y datos del integrante (columna derecha)
	\begin{center}
		\begin{tabular}{c c}
			\begin{tabular}{c}
				\includegraphics[height=3cm]{JAIME.jpg} \\
				\textbf{Jimenez Perez},\\ Jaime Jesus \\ \texttt{L21330603@hermosillo.tecnm.mx} \\ Teléfono: 623 106 3654
			\end{tabular} &
			\begin{tabular}{c}
				\includegraphics[height=3cm]{AXEL.jpg} \\
				\textbf{Montaño Maldonado,}\\ Axel Ariel \\ \texttt{l21330641@hermosillo.tecnm.mx} \\ Teléfono: 662 315 3681
			\end{tabular} \\ \vspace{2em}
			\begin{tabular}{c}
				\includegraphics[height=3cm]{RODRI.jpg} \\
				\textbf{Oloño Rivera,}\\Jose Rodrigo \\ \texttt{l21330660@hermosillo.tecnm.mx} \\ Teléfono: 662 139 6668
			\end{tabular} &
			\begin{tabular}{c}
				\includegraphics[height=3cm]{EMILIANO.jpg} \\
				\textbf{Valenzuela Esquer,}\\ Emiliano \\ \texttt{L21330706@hermosillo.tecnm.mx} \\ Teléfono: 662 934 3654
			\end{tabular}
		\end{tabular}
	\end{center}

\end{titlepage}